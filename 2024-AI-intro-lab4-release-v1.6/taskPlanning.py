...
def readArgparse():
    ...
    parser.add_argument('--test_idx', type=int, default=11)
...
class PlanningController:
    def __init__(self, scene: Scene2D, eat_seq: List[int], max_step:int) -> None:
    ...
    def change_traj(self):
        ...
        if self.num_eaten < self.num_food:
            ...
            self.planner.find_path(self.physice_info.pacman_position(), next_food)
            #self.planner.draw_path(self.planner.path)
        else:
            # 全部吃完了
...
def task_planning_and_control(scene: Scene2D, eat_sequence: List[int], max_step:int):
    controller = PlanningController(scene, eat_sequence, max_step)
...
if __name__ == "__main__":
    ...
    if not args.no_render:
        ...
        task_planning_and_control(scene, eat_seq, args.max_step)
        viewer = SimpleViewer(True, scene)
        viewer.update_food_flag = True
        viewer.run()
        #viewer.scene.controller.planner.draw_path()
    else:
        seq_file = f'data_q3/seq{args.test_idx}.txt'
        with open(seq_file, 'r') as file:
            lines = file.readlines()
        ...
        testenv.run()
        #testenv.scene.controller.planner.draw_path()
